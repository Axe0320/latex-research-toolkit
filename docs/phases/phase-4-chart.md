...
- Sentryの適用範囲をアプリ全体に拡大（`main.tsx`でのinit、[03-tech-alignment.md](../03-tech-alignment.md)）。**統合アプリ用にSentryプロジェクトを新規作成し、Dの旧プロジェクトのDSNは流用しない**（旧4デプロイが生き続けるため、エラーの発生元を混同しないための分離）
