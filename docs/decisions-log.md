...
### 2026-07-06 — Phase 8着手前：`04-risks.md`の未消化チェックリスト2件に対応

Phase 7完了後、Phase 8（デプロイ）に進む前に`04-risks.md`「実装前の最終チェックリスト」を再確認したところ、計画時から未対応のまま残っていた2項目を発見。ユーザーに確認の上、両方とも今のうちに対応する方針とした。

**1. `/api/resolve-citation`の実装**：[01-architecture.md](01-architecture.md) §2.6で計画されていた（Citationモジュールの外部公開CORSプロキシ依存を自前プロキシに置き換える改善）にもかかわらず、Phase 1〜7を通じて未実装のままだった。今回`api/resolve-citation.py`を新設し、`fetchCitation.ts`の`tryFetchHTML()`をこれを呼ぶよう変更。SSRF対策として、許可リスト（`doi.org`/`dx.doi.org`/`dl.acm.org`/`link.springer.com`/`ieeexplore.ieee.org`/`sciencedirect.com`、HTTPSスキームのみ）でホストを制限し、リダイレクト先も同じ許可リストで再検証する（許可ドメインからの別ドメインへのリダイレクトでの回避を防止）。`scripts/dev-api-server.py`・`vite.config.ts`にもポート8805のルートを追加し、他のPython関数と同じくローカルで動作確認できるようにした。IEEE Xplore宛のリクエストで実際にAWS WAFのボット判定ページが返る（`citation_doi`メタタグが取得できない）ことを確認したが、これは既存READMEに記載済みの既知の制限（サイト側のボット対策）であり、旧corsproxy.io実装でも同様に発生していたはずの制限であって今回の実装による劣化ではない。

**2. IndexedDBスキーマバージョニング規約の適用範囲**：`shared/persistence`の`schemaVersion`+`migrate()`規約はPhase 1〜7を通じてTableの`tableSessions`にしか適用されておらず、Citationの`citationLibrary`・Chartの`figures`/`layout`/`previews`は未対応だった。ユーザーは「今のうちに全ストアに適用する」を選択したが、実装時に精査した結果、`figures`ストアは`FigureType`ごとに分岐する判別共用体（16種類）が全コンポーネント・APIに深く埋め込まれており、全バリアントに`schemaVersion`フィールドを追加する変更は影響範囲が広く実装コストに見合わないと判断。`previews`もopaqueなbase64画像文字列でスキーマ自体を持たない。そのため実際に適用したのは**Citationの`citationLibrary`とChartの`layout`のみ**とし、`figures`は既存の構造検出型マイグレーション（`migrateFigures()`、例：`if (!('series' in d))`でscatter_plotの旧形式を検出）を踏襲する方針に変更した。この判断はユーザーの指示を字面通りには実行していないため、実装後に理由とともに報告し記録する。

### 2026-07-06 — テストフィクスチャの配置をtest-data/へ集約

Phase 7完了後、コピーしたサンプルテストデータの配置についてユーザーから「test フォルダを作ってまとめておくのかと思った」という指摘を受けた。当初は各モジュールの`src/**/__tests__/fixtures/`に分散配置していたが、`repos/`自体がGitHubにpushされない参照用クローンであること（＝統合後のリポジトリ単体で見るとこのサンプルデータの由来が分かりにくくなる）を踏まえ、プロジェクト直下の`test-data/{table,figure-convert,chart}/`に一本化した（元の4リポジトリがそれぞれ1つの`test/`・`test-data/`フォルダを持っていた構成に近い）。`git mv`でリネームとして移動し、参照元テストファイル3件のパスを更新。Citation用の`test-data/citation/`が無い点についても確認：Citationは唯一、サンプルデータではなく本物の自動テストコード（`pr1234.test.ts`等）を最初から持っていたリポジトリで、テストデータはコード内にインラインで埋め込まれておりフィクスチャファイルへの外部切り出しが元々不要なため、4フォルダ目は作成していない。

### 2026-07-06 — Phase 8着手前の最終点検：react-hooks違反の修正とクロスブラウザ確認

Phase 8に進む前の点検として`npx oxlint`を実行したところ、`ChartModule.tsx`に`react-hooks/rules-of-hooks`の`error`レベル違反を2件発見。`makeParamsHandler`・`makeReset`という素の（`use`接頭辞を持たない）ヘルパー関数が内部で`useCallback`を呼び出しており、ESLintのReact Hooksプラグインがフックの呼び出し順序の安全性を静的検証できない状態だった。実際には16図種ぶん固定回数・固定順序で呼ばれているため現時点で実害はないが、将来この呼び出しが条件分岐やループに変わった場合に壊れるリスクがあるため、`useParamsHandler`・`useReset`にリネームして解消（カスタムフックとして正しく認識されるようにしただけで、ロジック自体は変更なし）。

あわせて、`04-risks.md`で未確認のままだった「ブラウザ互換性」項目についてPlaywrightでChromium/Firefox/WebKitの3エンジンによる全4タブのスモークテストを実施し、コンソールエラー無し・レンダリング崩れ無しを確認した（実機Safariではなく近似のWebKitエンジンでの確認）。`npm audit`は0件、`console.log`/`debugger`/`TODO`/`FIXME`の残存も無し。詳細：本コミットのdiff。

### 2026-07-06 — Phase 8着手前：エラーバウンダリを実装（03-tech-alignment.mdの積み残し）

ユーザーから「機能面で追加すべきものはないか」と問われ、計画ドキュメント全体（`INTEGRATION_PLAN.md`・`02-integrations.md`・`03-tech-alignment.md`）を再点検したところ、`03-tech-alignment.md`で「モジュールごとに`ErrorBoundary`でラップし、1モジュールの不具合が他タブを巻き込まないようにする」と明記されていたにもかかわらず、Phase 1〜7を通じて未実装（`ErrorBoundary`/`componentDidCatch`の実装がコードベースに一切存在しない）のまま残っていたことを発見。
