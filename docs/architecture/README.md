...
「システム全体」図では、色分けだけで区別できるノード群には枠を付けていない（例：入力ノードはすべて紫色なので、それだけで「入力」だとわかる）。枠を付けるのは、**色の異なる複数ノードを1つの意味的なまとまり**として示す必要がある場合のみ（例：chart.md の「バックエンド」枠は、オレンジのAPI呼び出しと緑のレンダラーをまとめて「Vercel Functions上で動く」ことを示している）。
